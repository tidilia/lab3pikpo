...
class DataProcessor(ABC):
    ...
    @abstractmethod
    def process_data(self, parameter: ParameterObject):
        pass

    # Точка запуска методов обработки данных
    @abstractmethod
    def run(self, parameter: ParameterObject):
        pass
...
class CsvDataProcessor(DataProcessor):
    ...
    def process_data(self, parameter: ParameterObject):
        if parameter.category is not None:
            self.result = self._dataset[self._dataset[self._dataset.columns[0]] == parameter.category]
        if parameter.productName is not None:
            self.result = self.result[self.result[self._dataset.columns[1]] == parameter.productName]
        if parameter.availability is not None:
            self.result = self.result[self.result[self._dataset.columns[3]] == parameter.availability]
        if parameter.sortResult is not None:
            self.result = self.sort_data_by_col(self.result, 'Оборачиваемость', True)

    #    Сортируем данные по значениям колонки "LKG" и сохраняем результат в атрибуте result
    def run(self, parameter: ParameterObject):
        self.process_data(parameter)
